...
{
  if (!(push16(pusch_pdu->pdu_bit_map, ppWritePackedMsg, end)
...
static uint8_t unpack_ul_tti_request_pucch_pdu(void *tlv, uint8_t **ppReadPackedMsg, uint8_t *end)
{
  ...
  uint16_t dummy16 = 0;
  uint8_t dummy8 = 0;
  return (pull16(ppReadPackedMsg, &pucch_pdu->rnti, end)
...
static uint8_t unpack_ul_tti_request_pusch_pdu(void *tlv, uint8_t **ppReadPackedMsg, uint8_t *end)
{
  ...
  if (!(pull16(ppReadPackedMsg, &pusch_pdu->pdu_bit_map, end)
        ...
        && pull16(ppReadPackedMsg, &pusch_pdu->pusch_identity, end)
