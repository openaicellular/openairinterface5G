                           ...
                           sub_frame_t slot)
{
  ...
  UE_iterator(nrmac->UE_info.list, UE ) {
    ...
    NR_UE_UL_BWP_t *ul_bwp = &UE->current_UL_BWP;
    const int n_slots_frame = nr_slots_per_frame[ul_bwp->scs];
    if (nr_timer_is_active(&sched_ctrl->transmission_stop) || (sched_ctrl->ul_failure && !get_softmodem_params()->phy_test)) {
      continue;
    }
...
void nr_sr_reporting(gNB_MAC_INST *nrmac, frame_t SFN, sub_frame_t slot)
{
  ...
  UE_iterator(nrmac->UE_info.list, UE) {
    ...
    NR_UE_UL_BWP_t *ul_bwp = &UE->current_UL_BWP;
    const int n_slots_frame = nr_slots_per_frame[ul_bwp->scs];
    if (sched_ctrl->ul_failure || nr_timer_is_active(&sched_ctrl->transmission_stop))
      continue;
    NR_PUCCH_Config_t *pucch_Config = ul_bwp->pucch_Config;
